...
public class HeapPage implements Page {
    ...
    private boolean isDirty = false;
     ...
     // Track which transaction dirty the page
    ...
    */
    private int getNumTuples() {   
        int tupleSize = td.getSizeInBytes(); // Get tuple size from TupleDesc
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        // ensure record ids pageIds match
    ...
    public boolean isSlotUsed(int i) {
        int byteIndex = i / 8; // get byte in the header
        int bitIndex = i % 8;  // get bit in that byte
        return (header[byteIndex] & (1 << bitIndex)) != 0; // Check that bit within the header's bytes
    }
    ...
    private void markSlotUsed(int i, boolean value) {
        int byteIndex = i / 8; // Calculate which byte in the header
        int bitIndex = i % 8;  // Calculate which bit in that byte

        if (value) {
            header[byteIndex] |= (1 << bitIndex); // Set the bit to 1
        } else {
            header[byteIndex] &= ~(1 << bitIndex); // Set the bit to 0
        }
    }
    ...
    public Iterator<Tuple> iterator() {
        return new Iterator<Tuple>() {
            ...
            public boolean hasNext() {
                // Find the next used slot with a tuple
                while (currentIndex < numSlots && !isSlotUsed(currentIndex)) {
                    currentIndex++;
                }
            ...
            @Override
            public Tuple next() {
                // Ensure there is a next tuple to return
                if (!hasNext()) {
                    throw new NoSuchElementException("No more tuples in the page.");
                }
                
                // Return the tuple and increment the index for the next call
                Tuple nextTuple = tuples[currentIndex];
                currentIndex++; // Move to the next slot for the next iteration
                return nextTuple;
            }
